...
import com.nic.gajacyclonebeneficiarysurvey.databinding.LoginScreenBinding;
...
import com.nic.gajacyclonebeneficiarysurvey.utils.FontCache;
...
public class LoginScreen extends AppCompatActivity implements View.OnClickListener, Api.ServerResponseListener {
    ...
    public class InsertVillageTask extends AsyncTask<JSONObject, Void, Void> {
        ...
        protected Void doInBackground(JSONObject... params) {
            dbData.open();
            ArrayList<GajaCycloneBeneficiarySurvey> villagelist_count = dbData.getAll_Village(prefManager.getDistrictCode(),prefManager.getBlockCode());
    ...
    public class InsertHabTask extends AsyncTask<JSONObject, Void, Void> {
        ...
        protected Void doInBackground(JSONObject... params) {
            dbData.open();
            ArrayList<GajaCycloneBeneficiarySurvey> hablist_count = dbData.getAll_Habitation(prefManager.getDistrictCode(),prefManager.getBlockCode());
            if (hablist_count.size() <= 0) {
                if (params.length > 0) {
                    ...
                    for (int i = 0; i < jsonArray.length(); i++) {
                        GajaCycloneBeneficiarySurvey habListValue = new GajaCycloneBeneficiarySurvey();
                        try {

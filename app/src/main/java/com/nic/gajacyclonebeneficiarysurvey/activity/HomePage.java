...
import com.nic.gajacyclonebeneficiarysurvey.adapter.CommonAdapter;
...
import com.nic.gajacyclonebeneficiarysurvey.databinding.HomeScreenBinding;
import com.nic.gajacyclonebeneficiarysurvey.dialog.MyDialog;
...
import com.nic.gajacyclonebeneficiarysurvey.support.ProgressHUD;
...
public class HomePage extends AppCompatActivity implements Api.ServerResponseListener, View.OnClickListener, MyDialog.myOnClickListener {
    ...
    private List<GajaCycloneBeneficiarySurvey> Village = new ArrayList<>();
    private List<GajaCycloneBeneficiarySurvey> Habitation = new ArrayList<>();
...
//    }
    ...
    public void villageFilterSpinner(String filterVillage) {
        ...
        if (VillageList.getCount() > 0) {
            if (VillageList.moveToFirst()) {
                do {
                    GajaCycloneBeneficiarySurvey villageList = new GajaCycloneBeneficiarySurvey();
    ...
    public void habitationFilterSpinner(String dcode,String bcode, String pvcode) {
        ...
        Habitation.clear();
        GajaCycloneBeneficiarySurvey habitationListValue = new GajaCycloneBeneficiarySurvey();
        ...
        if (HABList.getCount() > 0) {
            if (HABList.moveToFirst()) {
                do {
                    GajaCycloneBeneficiarySurvey habList = new GajaCycloneBeneficiarySurvey();
    ...
    public void logout() {
        dbData.open();
        ArrayList<GajaCycloneBeneficiarySurvey> ImageCount = dbData.getSavedPMAYDetails();
        if (!Utils.isOnline()) {
...
//            startActivity(getIntent());
    ...
    public void syncButtonVisibility() {
        dbData.open();
        ArrayList<GajaCycloneBeneficiarySurvey> workImageCount = dbData.getSavedPMAYDetails();
    ...
    public class InsertPMAYTask extends AsyncTask<JSONObject, Void, Void> {
        ...
        protected Void doInBackground(JSONObject... params) {
            ...
            dbData.open();
            ArrayList<GajaCycloneBeneficiarySurvey> all_pmayListCount = dbData.getAll_PMAYList("","");
            if (all_pmayListCount.size() <= 0) {
                if (params.length > 0) {
                    ...
                    for (int i = 0; i < jsonArray.length(); i++) {
                        GajaCycloneBeneficiarySurvey pmaySurvey = new GajaCycloneBeneficiarySurvey();
                        try {
